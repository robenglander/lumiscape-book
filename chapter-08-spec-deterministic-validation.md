...
The skill defines a domain-specific list of components that the validation must cover. For the example system, these include: money conventions (cents rounding), interest and growth calculators, withdrawals, RMD calculations, taxes, inflation application, ledger accounting, validation and error handling, and reporting. The component list lives in the PROJECT CONFIGURATION block of the skill so it can be replaced for a different domain without changing the validation methodology.
