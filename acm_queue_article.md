...
A second pass operates across the entire specification surface with a different methodology: evidence-first mechanical search. A specification can pass individual review and still be wrong in context. This pass detects dependent specification drift, stale vocabulary, count mismatches, and conflicting assertions across specifications. No consistency assertion is accepted without showing the data. Terms changed in a normative source must cascade to every specification that depends on them. A vocabulary extended in one specification and not updated in its dependents is a correctness failure waiting to be compiled.
