...
**Behavioral Coherence (Mode 2c).** Mode 2b closes the interpretation boundary against mechanical inconsistency: stale vocabulary, conflicting assertions, dependent spec drift, count mismatches. But a specification surface can pass every mechanical check and still describe a system that cannot be implemented correctly — because the contracts at data handoffs between specifications are wrong. Mode 2c addresses this through path tracing rather than search. It reads a specification, identifies its outputs in the Architecture Metadata dependency graph, reads the consuming specification, and verifies the contract at the handoff: same types, same nullability, same semantics, same constraints. The failures it surfaces — contract mismatches, semantic disagreements, lifecycle gaps, ordering conflicts — are not visible to any grep. They only emerge when the data flow is traced end to end. Mode 2b runs when vocabulary changes. Mode 2c runs when contracts need tracing. Together they close the interpretation boundary at both the mechanical and behavioral levels.
...
**Traceability (Mode 9).** Mode 9 produces the traceability matrix: the observable artifact chain from requirement ID to acceptance test to production code module to unit test. A gap in the matrix — a Behavior ID with no code location — is visible evidence of incomplete correctness. Mode 9 also performs mutation resilience analysis: for each high-priority test, it asks whether a common one-character mutation to the production code would cause the test to fail. Tests that cannot detect mutation are strengthened. Finally, Mode 9 verifies CI gate configuration — in particular, that the build will reject any placeholder assertions, ensuring the artifact chain cannot be silently voided by vacuous tests accumulating over time.
...
graph LR
    ...
    M2 --> M2b["Deep Review"]
    M2b --> M2c["Behavioral Coherence"]
    M2c --> M3["Spec Freeze"]
    M3 --> M4["Deterministic Validation"]
    M4 --> M5["Stochastic Validation"]
    M5 --> M6["Integration Validation"]
    M6 --> M7["Test Generation"]
    M7 --> M8["Governed Implementation"]
    M8 --> M8b["Alignment Check"]
    M8b --> M9["Traceability"]
```
